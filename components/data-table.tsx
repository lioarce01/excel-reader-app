...
interface DataTableProps {
  data: { nombre: string; code1: string; code2: string; build: BuildData | null }[]
  loading: boolean
...
export function DataTable({ data, loading, onDelete, startIndex = 0 }: DataTableProps) {
  ...
  const handleViewBuild = (build: BuildData, name: string) => {
  ...
  return (
    <>
                  ...
                  <div className="flex items-center justify-center gap-1">
                    {row.build && (
                      <Button
                        ...
                        onClick={() => handleViewBuild(row.build!, row.nombre)}
                        title="Ver Build"
                      >
      ...
      <BuildModal
        ...
        characterName={selectedBuild?.name || ""}
      />
    </>
  )
}
